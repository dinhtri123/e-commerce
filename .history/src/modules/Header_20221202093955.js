  ...
  return (
    <div className="relative">
      <div className="fixed top-0 left-0 w-full z-10 isolute header bg-white">
        <div className="container mx-auto px-5 max-sm:px-3 max-sm:py-1 sm:grid sm:grid-cols-[1fr,2fr,1fr] md:grid-cols-3 flex items-center w-full sm:h-[70px] h-[60px] max-sm:bg-white max-sm:shadow-lg">
          ...
          <div className="max-sm:hidden flex gap-x-2 items-center justify-center">
            {menuLinks.map((link) => (
              <NavLink
                to={link.path}
                key={link.title}
                className={`font-medium text-[15px] text-blueDark py-3 px-5 lg:h-[85%] sm:h-[90%] flex items-center justify-center rounded-[30px] active-tablet`}
              >
                {link.title}
              </NavLink>
            ))}
          ...
          <div className="flex items-center justify-end cursor-pointer gap-x-5">
            ...
            <div>
              {user ? (
                ...
                <NavLink
                  to={"/login"}
                  className="rounded-md bg-blueDark text-white py-3 px-6"
                >
                  Login
                </NavLink>
              )}
