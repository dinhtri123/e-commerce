...
const Header = () => {
  ...
  return (
    ...
    <div className="fixed top-0 left-0 w-full z-10 isolute header">
        ...
        <div className="flex items-center justify-end cursor-pointer gap-x-5">
          ...
          <div>
            {user ? (
              ...
              <NavLink to={"/login"} className='rounded-md bg-primary text-white py-3 px-6'>Login</NavLink>
            )}
          </div>
        </div>
      </div>
    ...
    {profile && (
      <div className="z-50 isolute fixed top-[75px] right-10 bg-white text-black rounded-lg p-5 flex flex-col gap-5">
        <p className="flex gap-2"><span>Username:</span> <span className="font-medium">{user.displayName}</span></p>
        <p className="flex gap-2"><span>Email:</span><span className="font-medium">{user.email}</span></p>
        <NavLink
          to={"/signup"}
