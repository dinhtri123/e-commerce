          ...
          <div>
            {user ? (
              <div className="relative">
                ...
                {profile && 
                <div className="absolute top-[100%] right-0 bg-primary text-white rounded-lg p-5">
                <p>Username: {user.displayName}</p>
                <p>{}</p>
                </div>
                }
                <NavLink to={"/signup"} onClick={handleSignOut} className='rounded-md bg-error text-white flex items-center gap-x-1 py-3 px-4'>
                  <span>Logout</span>
                  <img src="/logout.png" alt="" className="w-5 h-5 object-cover" />
                </NavLink>
              </div>
            ) : (
