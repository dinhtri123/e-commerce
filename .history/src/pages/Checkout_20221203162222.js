              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex gap-5 items-center max-h-0 overflow-hidden delivery opacity-0">
                  {delivery.map((item, index) => (
                    <div
                      key={index}
                      className={`flex items-center justify-center w-[160px] h-[50px] font-medium text-text3 text-[15px] gap-x-2 border rounded-xl cursor-pointer delivery-item ${activeDelivery === item.title ? }`}
                    >
                      {item.icon}
