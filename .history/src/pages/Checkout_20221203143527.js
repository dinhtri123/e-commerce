...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('load', handleCollapse)
    }
  },[])
  const handleCollapse = () => {
    const collapse = document.querySelectorAll('.accordion');
    for(let i = 0; i < collapse.length; i++) {
      collapse[i].addEventListener('click', (e) => {
        e.classList.toggle("collapse-active")
      })
    }
  }
  return (
          ...
          <div>
            ...
            <div className="flex flex-col gap-5">
              <div className="flex flex-col gap-2">
                <div className="w-full h-full border rounded-lg px-5 py-3 flex items-center justify-between cursor-pointer accordion">
                ...
                <form
                  className="grid grid-cols-2 gap-5 text-sm rounded-lg bg-white border max-h-0 items-start overflow-hidden collapse-item"
                  autoComplete="off"
                >
                  <Input
                    text={"Name*"}
