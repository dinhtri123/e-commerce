...
const Login = () => {
    const {showPass, handleShowPassword} = useToggle();
    const showPassword = () => {
        showPassword(true);
    }
  return (
        ...
        <div className="login max-w-[500px] w-full mx-auto p-10 flex flex-col items-center">
          ...
          <form autoComplete="off" className="flex flex-col gap-5 w-[90%]">
            ...
            <div className="flex flex-col gap-2">
              ...
              />
              {showPass ? <IconEyeSlash></IconEyeSlash> : ""}
              <IconEye className={`absolute top-2/4 right-5 -translate-y-2/4 cursor-pointer`} onClick={showPassword}></IconEye>
              </div>
            </div>
