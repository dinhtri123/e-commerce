...
const SignUp = () => {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      const storageRef = ref(storage, `images/${Date.now() + values.name}`);
      const uploadTask = uploadBytesResumable(storage, values.file);
        uploadTask.on((error)=>{
            toast.error(error.message)
        }, ()=> {
            getDownloadURL(uploadTask.snapshot.task)
        })
      console.log(user);
    } catch (error) {
  ...
  return (
    <div className="w-full h-full bg-primary bg-opacity-10">
      <div className="container w-full h-full p-5 relative flex justify-center items-center">
