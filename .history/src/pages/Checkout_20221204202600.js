...
const Checkout = () => {
  ...
  const onSubmit = (values) => {
    if (activeDelivery === "") {
      ...
      if(!isValid) return;
      localStorage.setItem("order-info", JSON.stringify(values, activeDelivery));
      localStorage.setItem('order-product', JSON.stringify(cartItems));
      if(    formState: { errors, isValid, isSubmitting },
        ) {
